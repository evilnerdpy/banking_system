...
from database_app.database_functions import deposit_and_withdrawal_money_on_current_account
from database_app.database_functions import get_user_current_wallet


def user_current_menu(user):
    while True:
        user_current_wallet = get_user_current_wallet(user[0])
        if user_current_wallet == -1:
            add_new_user_in_current_wallets_database(user[0])
            user_current_wallet = get_user_current_wallet(user[0])
        print(f"Dear,{user[0]}!\n"
              f"This is the menu for operations with the current wallet!\n"
...
def check_if_withdrawal_value_is_in_right_form(withdrawal, user_balance):
    try:
        if ((user_balance - float(withdrawal)) > 500) and float(withdrawal) > 0:
            return True
        print("Not enough money on the balance.\n"
              "Minimum balance for current wallet is 500RM")
        return False
    except ValueError:
        return False
...
def user_deposit_menu(user):
    ...
    if not check_if_deposit_value_is_in_right_form(deposit_value):
        print("Invalid input, please try again")
        return
    deposit_and_withdrawal_money_on_current_account(user[0], deposit_value, "deposit")
...
def user_withdrawal_menu(user, balance):
    ...
    if not check_if_withdrawal_value_is_in_right_form(withdrawal_value, balance):
        print("Invalid input, please try again")
        return
    deposit_and_withdrawal_money_on_current_account(user[0], withdrawal_value, "withdrawal")
